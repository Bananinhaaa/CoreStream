  ...
  if (isGenerating) {
    return (
      <div className="h-full bg-black flex flex-col items-center justify-center p-10 text-center animate-view">
        ...
        <h2 className="text-xl font-black italic uppercase tracking-tighter mb-4">Quantum Rendering</h2>
        <p className="text-[10px] text-gray-500 uppercase tracking-[0.3em] leading-relaxed max-w-xs">
          O modelo Veo 3.1 está processando sua imaginação. Isso pode levar até 2 minutos.
        </p>
      </div>
    );
  }
    ...
    <div className="h-full animate-view bg-black flex flex-col overflow-hidden pb-24 px-6">
      <div className="flex justify-center pt-8 pb-4">
        <div className="bg-white/5 p-1.5 rounded-2xl flex gap-1 border border-white/5">
           <button onClick={() => setMode('upload')} className={`px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${mode === 'upload' ? 'bg-white text-black' : 'text-gray-500'}`}>Upload</button>
        ...
        <div className="space-y-6 pt-4 max-w-md mx-auto">
          {mode === 'ai' ? (
            <div className="bg-indigo-600/10 border border-indigo-500/20 p-8 rounded-[2.5rem] space-y-6">
               ...
               <textarea 
                  placeholder="Ex: Um scripter cyberpunk hackeando em uma sala cheia de hologramas neon..."
                  value={aiPrompt}
            ...
            <>
              ...
              <div className="bg-white/5 p-6 rounded-3xl border border-white/10 relative">
                ...
                <textarea placeholder="Escreva algo..." value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-transparent text-sm outline-none h-20 resize-none text-white placeholder:text-gray-700" />
              </div>
